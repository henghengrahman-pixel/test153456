...
test('v1.18.2 bonus flow prioritizes current-message ID and accepts standalone requested ID',()=>{
  ...
  assert.match(eng,/const uid=extractUserIdFromText\(text\) \|\| extractUserId\(ctx\)/);
  assert.match(eng,/extractUserIdFromText\(text\) \|\| extractUserId\(ctx\) \|\| extractStandaloneRequestedUserId\(ctx\)/);

...
let currentChat=null,currentTitle='',currentChatMode='AI',liveUiBusy=false,liveUiTimer=null,chatListFingerprint='',cannedCache=[],editingResponseId=null,promoCache=[],editingPromoId=null,importantCache=[],editingImportantId=null,importantFilter='ALL';
...
async function openChat(id,title,auto=false){
  currentChat=id;currentTitle=title||currentTitle;$('#chatTitle').textContent=currentTitle;if($('#chatAvatar'))$('#chatAvatar').textContent=(currentTitle||'?').trim().slice(0,1).toUpperCase()||'?';if($('#endChatBtn'))$('#endChatBtn').disabled=false;renderChatMode();
  ...
  renderCaseMeta(d);
  const firstAt=d.items?.[0]?.created_at?new Date(d.items[0].created_at):null;
  ...
  const html=started+d.items.map(m=>{
...
$('#endChatBtn').onclick=async()=>{if(!currentChat)return;if(!confirm(`End chat ${currentTitle||currentChat}? Chat akan ditutup juga di LiveChat.`))return;const b=$('#endChatBtn');b.disabled=true;try{await api(`/api/conversations/${encodeURIComponent(currentChat)}/end`,{method:'POST'});currentChat=null;currentTitle='';$('#chatTitle').textContent='Pilih chat';$('#chatAvatar').textContent='?';$('#typingState').textContent='';$('#messages').innerHTML='<div class="conversationempty">Chat sudah ditutup.</div>';currentChatMode='AI';renderChatMode();await loadChats()}catch(e){alert('Gagal end chat: '+e.message);b.disabled=false}};
...
function humanMessageHtml(m,name){
  ...
  const media=at.map(a=>a.isImage&&a.url?`<a href="${esc(a.url)}" target="_blank" rel="noreferrer"><img class="chatimg" src="${esc(a.url)}" alt="attachment"></a>`:(a.url?`<a href="${esc(a.url)}" target="_blank" rel="noreferrer">Lihat file</a>`:'')).join('');
  const type=String(m.sender_type||'unknown');
  const label=type==='customer'?(name||'Member'):type==='ai'?'CS Bot':type==='agent'?'CS':'System';
  const ts=m.created_at?new Date(m.created_at).toLocaleString('id-ID',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'}):'';
  return `<div class="msgrow ${esc(type)}"><div class="msg ${esc(type)}"><div class="who">${esc(label)}</div><div class="msgbody">${esc(m.text)}</div>${media?`<div class="attachments">${media}</div>`:''}${ts?`<div class="msgtime">${esc(ts)}</div>`:''}</div></div>`;
}
...
async function liveUiTick(){if(liveUiBusy)return;liveUiBusy=true;try{const tab=$('#tab-chats');if(tab.classList.contains('active')){await loadChats();if(currentChat)await openChat(currentChat,currentTitle,true)}if($('#tab-human').classList.contains('active'))await loadHuman()}catch{}finally{liveUiBusy=false}}

...
reset password ko`,requireTelegramDelivery:true});
  ...
  if(effective.includes('BONUS')){
    ...
    const uid=extractUserIdFromText(text) || (wf?.workflow_type==='BONUS_CLAIM' ? extractRequestedUserIdFromText(text) : '') || extractUserId(ctx) || (wf?.workflow_type==='BONUS_CLAIM' ? extractStandaloneRequestedUserId(ctx) : '');

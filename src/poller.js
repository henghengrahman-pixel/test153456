...
async function ensureFreshWelcomeGreeting(chatId, events, livechat){
  ...
  // processGreetingTrigger is idempotent by the banner event id. Calling it here is
  // intentional: the banner might already have been inserted by an earlier sync path,
  // but the greeting action may not have executed (restart/race/legacy deployment).

...
app.use('/assets',express.static(path.join(__dirname,'../public'),{maxAge:0,etag:true,setHeaders(res){res.setHeader('Cache-Control','no-store, max-age=0');}}));
...
app.post('/api/conversations/:id/send',requireAdmin,async(req,res)=>{const text=String(req.body?.text||'').trim();if(!text)return res.status(400).json({ok:false,error:'EMPTY_TEXT'});try{const out=await withChatLock(req.params.id,async()=>{const row=(await pool.query(`SELECT status,conversation_state,handling_mode FROM conversations WHERE chat_id=$1 LIMIT 1`,[req.params.id])).rows[0];if(!row){const er=new Error('CONVERSATION_NOT_FOUND');er.status=404;throw er;}if(String(row.status||'').toLowerCase()==='closed'||row.conversation_state==='CLOSED'){const er=new Error('CONVERSATION_CLOSED');er.status=409;throw er;}let autoTakeover=false;if(row.handling_mode!=='HUMAN'){await setHumanTakeover(req.params.id,'dashboard_manual_reply');autoTakeover=true;console.log(`[TAKEOVER] conversation=${req.params.id} reason=dashboard_manual_reply`);}const sent=await lc.sendMessage(req.params.id,text);const eventId=sent?.event_id||sent?.id||null;if(eventId){await insertMessage({chatId:req.params.id,eventId:String(eventId),senderType:'agent',authorId:'admin',text,normalizedText:normalizeText(text),intent:detectIntent(text),createdAt:new Date().toISOString()});await captureHumanReplyLearning({chatId:req.params.id,eventId:String(eventId),responseText:text}).catch(()=>{});}console.log(`[MESSAGE_SEND] conversation=${req.params.id} source=dashboard_human event=${eventId||'-'} auto_takeover=${autoTakeover}`);return {ok:true,sent,mode:'HUMAN',autoTakeover};});res.json(out);}catch(e){await logError('manual_reply','SEND_FAILED',e.message,{chatId:req.params.id}).catch(()=>{});res.status(e.status||502).json({ok:false,error:e.message});}});

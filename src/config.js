...
export const config = {
  ...
  lcMaxPages: Math.min(50, Math.max(1, int(process.env.LIVECHAT_MAX_PAGES, 10))),
  ...
  aiGlobalDefault: bool(process.env.AI_GLOBAL_ENABLED, true),
  ...
  holdingCooldownSeconds: Math.max(15, int(process.env.HOLDING_COOLDOWN_SECONDS, 60)),
  defaultSiteId: String(process.env.DEFAULT_SITE_ID || 'omtogel').trim().toLowerCase().replace(/[^a-z0-9_-]+/g,'-') || 'default',
  defaultSiteName: process.env.DEFAULT_SITE_NAME || 'OMTOGEL',
...
export function validateConfig() {
  ...
  if (!/^https:\/\//i.test(config.lcApiBase)) warnings.push('LIVECHAT_API_BASE harus HTTPS');

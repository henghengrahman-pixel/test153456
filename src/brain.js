...
export function applyBrainGate({intent='GENERAL',decision={},brain={},hasKnowledge=false}){
  ...
  if(b.contradictions.length){
    out.action='ESCALATE_HUMAN'; out.reply='';
  ...
  if(highRisk && ['SEND_MESSAGE','SEND_HOLDING_MESSAGE'].includes(out.action) && !hasKnowledge){
    out.action='ESCALATE_HUMAN'; out.reply=''; out.reason='brain_high_risk_without_verified_source';
    return out;
  }
  // Bila model tahu ada data yang benar-benar kurang, jangan menjawab seolah proses sudah lengkap.
  if(b.missingInfo.length && out.action==='SEND_MESSAGE' && !highRisk){

...
export class LiveChatClient {
  ...
  async listChats() {
    ...
    for (const body of candidates) {
      try {
        const first = await this.call('list_chats', body);
        const firstNorm = this.normalizeChatList(first);
        const items=[...firstNorm.items];
        let next=first?.next_page_id || first?.next_page || null;
        let pages=1;
        const seen=new Set(items.map(x=>String(x?.id||'')));
        while(next && pages<config.lcMaxPages){
          let page;
          try{ page=await this.call('list_chats',{...body,page_id:next}); }
          catch(e){ if([400,404,422].includes(Number(e?.status))) break; throw e; }
          const n=this.normalizeChatList(page);
          for(const item of n.items){const id=String(item?.id||'');if(!id||seen.has(id))continue;seen.add(id);items.push(item);}
          const prior=next; next=page?.next_page_id || page?.next_page || null; pages++;
          if(!next || String(next)===String(prior)) break;
        }
        return { ...first, _normalizedChats: items, _listSource: firstNorm.source, _pagesFetched:pages, found_chats:items.length, next_page_id:next||null };
      } catch (e) {
        last=e;
  ...
  async hasHumanReplyAfter(chatId, afterIso) {
    ...
    return events.some(ev=>{
      ...
      const author=String(ev.authorType||'').toLowerCase();
      return Number.isFinite(t) && t>after && author.includes('agent');
    });
  }

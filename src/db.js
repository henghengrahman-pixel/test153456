...
export async function archiveCurrentConversationSession(chatId,{excludeEventId=null,endedAt=null,reason='SESSION_CLOSED'}={}){
  ...
  let boundaryId=0, upperBoundId=null;
  if(excludeEventId){
    const current=await pool.query(`SELECT id FROM messages WHERE chat_id=$1 AND event_id=$2 LIMIT 1`,[chatId,String(excludeEventId)]);
    upperBoundId=Number(current.rows[0]?.id||0)||null;
    const b=upperBoundId
      ? await pool.query(`SELECT id FROM messages WHERE chat_id=$1 AND sender_type='system' AND intent='GREETING_TRIGGER' AND id<$2 ORDER BY id DESC LIMIT 1`,[chatId,upperBoundId])
      : await pool.query(`SELECT id FROM messages WHERE chat_id=$1 AND sender_type='system' AND intent='GREETING_TRIGGER' AND event_id<>$2 ORDER BY id DESC LIMIT 1`,[chatId,String(excludeEventId)]);
    boundaryId=Number(b.rows[0]?.id||0);
  }else{
    const b=await pool.query(`SELECT id FROM messages WHERE chat_id=$1 AND sender_type='system' AND intent='GREETING_TRIGGER' ORDER BY id DESC LIMIT 1`,[chatId]);
    boundaryId=Number(b.rows[0]?.id||0);
  }
  const params=[chatId,boundaryId];
  let extra='';
  if(upperBoundId){params.push(upperBoundId);extra=' AND id<$3';}
  else if(excludeEventId){params.push(String(excludeEventId));extra=' AND event_id<>$3';}
  const mRes=await pool.query(`SELECT id,event_id,sender_type,author_id,text,normalized_text,intent,attachments,created_at FROM messages WHERE chat_id=$1 AND id>=$2${extra} ORDER BY created_at ASC,id ASC`,params);
  const messages=mRes.rows;
  if(!messages.length) return null;
  const first=messages[0], last=messages[messages.length-1];
  const fallback=`legacy:${chatId}:${first.id}`;
  const sessionKey=String(c.session_key||fallback).trim()||fallback;
  ...
  const r=await pool.query(`INSERT INTO conversation_archives(chat_id,session_key,customer_name,customer_email,handling_mode,started_at,ended_at,close_reason,last_message,last_sender_type,agent_name,message_count,messages)
    ...
    ON CONFLICT(chat_id,session_key) DO UPDATE SET customer_name=EXCLUDED.customer_name,customer_email=EXCLUDED.customer_email,handling_mode=EXCLUDED.handling_mode,started_at=LEAST(conversation_archives.started_at,EXCLUDED.started_at),ended_at=GREATEST(conversation_archives.ended_at,EXCLUDED.ended_at),close_reason=EXCLUDED.close_reason,last_message=EXCLUDED.last_message,last_sender_type=EXCLUDED.last_sender_type,agent_name=COALESCE(EXCLUDED.agent_name,conversation_archives.agent_name),message_count=EXCLUDED.message_count,messages=EXCLUDED.messages,archived_at=now()
    RETURNING *`,[chatId,sessionKey,c.customer_name||null,c.customer_email||null,c.handling_mode||'AI',first.created_at,endedAt||last.created_at||new Date().toISOString(),String(reason||'SESSION_CLOSED'),String(last.text||'').slice(0,4000),last.sender_type||null,agentName,messages.length,JSON.stringify(messages)]);
  return r.rows[0]||null;
}

export async function listConversationArchives({q='',filter='ALL',limit=50,offset=0}={}){
  const safeLimit=Math.max(10,Math.min(Number(limit)||50,100));
  const safeOffset=Math.max(0,Number(offset)||0);
  const query=String(q||'').trim().slice(0,120);
  const mode=String(filter||'ALL').toUpperCase();
  const sql=`WITH archived AS (
      SELECT id::text AS archive_id,chat_id,session_key,customer_name,customer_email,handling_mode,started_at,ended_at,archived_at,close_reason,last_message,last_sender_type,agent_name,message_count,false AS legacy
      FROM conversation_archives
    ), legacy AS (
      SELECT ('legacy:'||c.chat_id) AS archive_id,c.chat_id,COALESCE(NULLIF(c.session_key,''),'legacy:'||c.chat_id) AS session_key,c.customer_name,c.customer_email,c.handling_mode,
        COALESCE((SELECT min(m.created_at) FROM messages m WHERE m.chat_id=c.chat_id),c.created_at) AS started_at,
        COALESCE(c.ended_at,(SELECT max(m.created_at) FROM messages m WHERE m.chat_id=c.chat_id),c.updated_at) AS ended_at,
        c.updated_at AS archived_at,'LEGACY_HISTORY'::text AS close_reason,
        (SELECT m.text FROM messages m WHERE m.chat_id=c.chat_id ORDER BY m.id DESC LIMIT 1) AS last_message,
        (SELECT m.sender_type FROM messages m WHERE m.chat_id=c.chat_id ORDER BY m.id DESC LIMIT 1) AS last_sender_type,
        (SELECT CASE WHEN m.sender_type='ai' THEN 'AI LIVECHAT 8008' ELSE m.author_id END FROM messages m WHERE m.chat_id=c.chat_id AND m.sender_type IN ('agent','ai') ORDER BY m.id DESC LIMIT 1) AS agent_name,
        (SELECT count(*)::int FROM messages m WHERE m.chat_id=c.chat_id) AS message_count,true AS legacy
      FROM conversations c
      WHERE c.visible_in_inbox=false AND EXISTS(SELECT 1 FROM messages m WHERE m.chat_id=c.chat_id) AND NOT EXISTS(SELECT 1 FROM conversation_archives a WHERE a.chat_id=c.chat_id)
    ), combined AS (SELECT * FROM archived UNION ALL SELECT * FROM legacy)
    SELECT *,count(*) OVER()::int AS total_count FROM combined
    WHERE ($1='' OR COALESCE(customer_name,'') ILIKE '%'||$1||'%' OR chat_id ILIKE '%'||$1||'%' OR COALESCE(last_message,'') ILIKE '%'||$1||'%')
      AND ($2='ALL' OR ($2='HUMAN' AND handling_mode='HUMAN') OR ($2='AI' AND handling_mode<>'HUMAN'))
    ORDER BY ended_at DESC NULLS LAST,archive_id DESC LIMIT $3 OFFSET $4`;
  const r=await pool.query(sql,[query,mode,safeLimit,safeOffset]);
  return {items:r.rows.map(({total_count,...row})=>row),total:Number(r.rows[0]?.total_count||0),limit:safeLimit,offset:safeOffset};
}

export async function getConversationArchive(archiveId){
  const key=String(archiveId||'');
  if(/^\d+$/.test(key)){
    const r=await pool.query(`SELECT * FROM conversation_archives WHERE id=$1 LIMIT 1`,[Number(key)]);
    return r.rows[0]||null;
  }
  if(!key.startsWith('legacy:')) return null;
  const chatId=key.slice(7);
  const cRes=await pool.query(`SELECT chat_id,customer_name,customer_email,handling_mode,created_at,ended_at,updated_at FROM conversations WHERE chat_id=$1 AND visible_in_inbox=false LIMIT 1`,[chatId]);
  const c=cRes.rows[0]; if(!c) return null;
  const mRes=await pool.query(`SELECT id,event_id,sender_type,author_id,text,normalized_text,intent,attachments,created_at FROM messages WHERE chat_id=$1 ORDER BY created_at ASC,id ASC`,[chatId]);
  if(!mRes.rowCount) return null;
  const messages=mRes.rows,first=messages[0],last=messages[messages.length-1];
  const agent=[...messages].reverse().find(m=>['agent','ai'].includes(String(m.sender_type||'').toLowerCase()));
  return {id:key,chat_id:chatId,session_key:'legacy:'+chatId,customer_name:c.customer_name,customer_email:c.customer_email,handling_mode:c.handling_mode,started_at:first.created_at||c.created_at,ended_at:c.ended_at||last.created_at||c.updated_at,archived_at:c.updated_at,close_reason:'LEGACY_HISTORY',last_message:last.text,last_sender_type:last.sender_type,agent_name:agent?.sender_type==='ai'?'AI LIVECHAT 8008':agent?.author_id||null,message_count:messages.length,messages,legacy:true};
}

export async function beginNewConversationSession(chatId,{triggerEventId=null,threadKey=null}={}){
  // Snapshot the previous service session before resetting transient state. This is read-only history
  // for /archives and does not alter the live conversation/message workflow.
  await archiveCurrentConversationSession(chatId,{excludeEventId:triggerEventId,endedAt:new Date().toISOString(),reason:'NEW_SESSION'}).catch(()=>{});
...
export async function markConversationEnded(chatId){
  await archiveCurrentConversationSession(chatId,{endedAt:new Date().toISOString(),reason:'END_CHAT'}).catch(()=>{});
